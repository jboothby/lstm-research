...
def predict_model(model_name, test_data):
    ...
    print(list(CLASSIFICATION.keys()))
    print(f'ypred={y_pred[:5]}, ytrue={y_true[:5]}')
...
"""
df = pd.read_csv(os.path.join(os.getcwd(), 'testing_data_360.csv'))
label = np.array(df.iloc[::10, :1])
label = np.array(list(map(lambda x: x if x in list(CLASSIFICATION_FULL.keys()) else 'Z', label.flatten())))
data = np.array(df.iloc[::10, 1:])
data = data.reshape(data.shape[0], data.shape[1], 1)
"""
...
# predict_model('LSTM_D45_L23_STEP1_Stratified.h5', (data, label))
predict_model_lite('LSTM_D45_L23_STEP1_Stratified.tflite', (data, label))

# predict_model('LSTM_64_32.h5', (data, label))
# predict_model_lite('LSTM_64_32.tflite', (data,label))

# predict_model('LSTM_90_45_norm.h5', (data, label))
# predict_model_lite('LSTM_90_45_norm.tflite', (data, label))
